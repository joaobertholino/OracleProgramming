...
public class JavaBankArrayListDelete extends JFrame {
	...
	private void CreateAccountJButtonActionPerformed(ActionEvent event) {
		...
		if ((Name != "") & (Accountnum != 0)) {
			...
			displayJTextArea.setText(Accounts.size() + " " + tempAccount.getAccountName() + " " + tempAccount.getAccountName()
		...
		if (Accounts.size() == 0) {
			...
			for (int i = 0; i < Accounts.size(); i++) {
				...
				if ((tempAccount.getAccountNum() == Accountnum) && (Deposit > 0)) {
					...
					displayJTextArea.setText(
							tempAccount.getAccountName() + " " + tempAccount.getAccountName() + " " + tempAccount.getBalance());
