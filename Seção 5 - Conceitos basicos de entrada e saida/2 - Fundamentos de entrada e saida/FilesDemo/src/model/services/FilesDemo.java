...
public class FilesDemo {
	 ...
	 * parâmetro {@code directoryPath}, recebendo o objeto da classe {@code Path} do parâmetro {@code filePath} como argumento.
	 * <p>
	 * Utilizando a estrutura condicional {@code if}, é verificado se o diretório do parâmetro {@code directoryPath} não existe,
	 ...
	 *
	 * @param directoryPath  Parâmetro que espera receber um objeto da classe {@code Path}, do qual representa um diretório
	 *                 absoluto.
	 * @param filePath Parâmetro que espera receber um objeto da classe {@code Path}, do qual representa um diretório
	 *                 relativo.
	 * @return Um objeto da classe {@code Path} que representa a resolução do objeto {@code directoryPath} e do objeto
	 * {@code filePath}.
	 * @throws IOException Propaga esta exceção para o escopo do qual este método é chamado.
	 */
	public static Path createFiles(Path directoryPath, Path filePath) throws IOException {
		Path absolutPath = directoryPath.resolve(filePath);
		if (Files.notExists(directoryPath)) {
			Files.createDirectories(directoryPath);
		}
		if (Files.notExists(absolutPath)) {
			Files.createFile(absolutPath);
		}
		return absolutPath;
	}
